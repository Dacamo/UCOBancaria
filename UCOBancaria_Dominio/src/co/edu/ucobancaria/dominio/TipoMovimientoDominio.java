...
public final class TipoMovimientoDominio {
	...
	public final static TipoMovimientoDominio CREAR(final int codigo, final String nombre, final String signo, final OperacionEnum operacion) {
		...
		case CONSULTAR:
			...
			break;
		default:
			String mensaje = "El objeto Tipo Movimiento no se puede crear, debido a que la operaci�n para validar su integridad no existe.";
	...
	private void asegurarIntegridadCodigo() {
		if (getCodigo() <= 0) {
			String mensaje = "El c�digo de un tipo de movimiento debe ser mayor a cero.";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
		}
	}
	...
	private void asegurarIntegridadNombre() {
		if (obtenerUtilTexto().cadenaEsVaciaONula(getNombre())) {
			String mensaje = "El nombre de un tipo de movimiento no puede estar vac�o.";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
		} else if (getNombre().trim().length() > 250) {
			String mensaje = "El nombre de un tipo de movimiento no tener m�s de 250 caracteres.";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
		} else if (!ExpresionRegularEnum.SOLO_TEXTO.cumplePatron(getNombre())) {
			String mensaje = "El nombre de un tipo de movimiento s�lo puede contener letras y espacios.";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
		}
	}
	...
	private void asegurarIntegridadSigno() {
		if (obtenerUtilTexto().cadenaEsVaciaONula(getSigno())) {
			String mensaje = "El signo de un tipo de movimiento no puede estar vac�o.";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
			...
			String mensaje = "El signo de un tipo de movimiento s�lo puede ser m�s (+) o menos (-).";
			throw AplicacionExcepcion.CREAR(mensaje, ExcepcionEnumeracion.DOMINIO);
		}
	}
}